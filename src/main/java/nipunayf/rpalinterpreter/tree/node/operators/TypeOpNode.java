...
public class TypeOpNode extends OperatorNode {
    ...
    public void execute(Stack<Node> stack) throws NoSuchMethodException, InvalidCSEMachineException {
        ...
        switch (OperatorDictionary.map.get(this.getValue())) {
            case IS_INTEGER:
                output = node.getType() == DataDictionary.Symbol.INTEGER;
                break;

            case IS_TRUTH_VALUE:
                output = node.getType() == DataDictionary.Symbol.BOOLEAN;
                break;

            case IS_STRING:
                output = node.getType() == DataDictionary.Symbol.STRING;
                break;

            case IS_TUPLE:
                output = node.getType() == DataDictionary.Symbol.TUPLE || node instanceof TauNode;
                break;

            case IS_DUMMY:
                output = node.getType() == DataDictionary.Symbol.DUMMY;
                break;
        ...
        stack.push(new DataNode(this.getLevel(), Boolean.toString(output), DataDictionary.Symbol.BOOLEAN));
    }
}
