...
public class TauOpNode extends OperatorNode {
    ...
    public void execute(Stack<Node> stack) throws InvalidCSEMachineException, NoSuchMethodException {
        ...
        && data.getType() != DataDictionary.Symbol.TUPLE)
            throw new InvalidCSEMachineException(this.getValue() + " operator only supports tuples");

        switch (this.getValue()) {
            case "Order":
                int size;
                if (data.getType() == DataDictionary.Symbol.TUPLE) {
                    size = 0;
                } else {
                    size = data.getChildren().size();
                }
                stack.push(new DataNode(this.getLevel(), Integer.toString(size), DataDictionary.Symbol.INTEGER));
                break;

            case "Null":
                stack.push((new DataNode(this.getLevel(), Boolean.toString(data.getValue().equals("nil")), DataDictionary.Symbol.BOOLEAN )));
                break;
